...
function StrHasOnlyValidTags(input) {
    ...
    let sanitizedInput = input
        .replace(/<em>|<\/em>|<strong>|<\/strong>|<sup>|<\/sup>|<sub>|<\/sub>|<u>|<\/u>|<hr>|<hr \/>|<br>|<br \/>|<p>|<\/p>|<a [^>]*>|<\/a>/gi, "");

    // Check if any other tags exist in the sanitized string
    return !/<[^>]*>/g.test(sanitizedInput);
}
...
function EPAlertWithDetails(msg, details){
    EPAlert(msg+'<br><br>'+GetEPShowDetails(details));
}

function EPAlert(msg){
    EPConfirm('Alert', msg, function(){}, '#555', false);
}
...
function EPConfirm(title, msg, callback, color, showcancel){
    let colorStyle = '#555';
